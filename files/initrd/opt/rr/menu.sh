...
function downloadExts() {
  ...
  if [ "${PRERELEASE}" = "true" ]; then
    ...
    # TAG=`curl -skL "${PROXY}https://api.github.com/repos/john-shine/rr-ng-addons/releases/latest" | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
...
function updateMenu() {
  while true; do
    ...
    a)
      ...
      downloadExts "addons" "${CUR_ADDONS_VER:-None}" "https://github.com/john-shine/rr-ng-addons" "addons" "1"
      [ $? -eq 0 ] && updateExts "addons" "1"
      T="$(printf "$(TEXT "Update %s")" "$(TEXT "modules")")"
      downloadExts "modules" "${CUR_MODULES_VER:-None}" "https://github.com/john-shine/rr-ng-modules" "modules" "1"
      [ $? -eq 0 ] && updateExts "modules" "1"
      T="$(printf "$(TEXT "Update %s")" "$(TEXT "LKMs")")"
      downloadExts "LKMs" "${CUR_LKMS_VER:-None}" "https://github.com/john-shine/rr-ng-lkms" "rp-lkms" "1"
      [ $? -eq 0 ] && updateExts "LKMs" "1"
      T="$(printf "$(TEXT "Update %s")" "$(TEXT "RR")")"
      downloadExts "RR" "${CUR_RR_VER:-None}" "https://github.com/john-shine/rr-ng" "update" "0"
      [ $? -ne 0 ] && continue
      updateRR "RR"
      ;;
    ...
    d)
      T="$(printf "$(TEXT "Update %s")" "$(TEXT "addons")")"
      downloadExts "addons" "${CUR_ADDONS_VER:-None}" "https://github.com/john-shine/rr-ng-addons" "addons" "0"
      [ $? -ne 0 ] && continue
      updateExts "addons" "0"
      ;;
    ...
    m)
      T="$(printf "$(TEXT "Update %s")" "$(TEXT "modules")")"
      downloadExts "modules" "${CUR_MODULES_VER:-None}" "https://github.com/john-shine/rr-ng-modules" "modules" "0"
      [ $? -ne 0 ] && continue
    ...
    l)
      T="$(printf "$(TEXT "Update %s")" "$(TEXT "LKMs")")"
      downloadExts "LKMs" "${CUR_LKMS_VER:-None}" "https://github.com/john-shine/rr-ng-lkms" "rp-lkms" "0"
      [ $? -ne 0 ] && continue
      updateExts "LKMs" "0"
      ;;
